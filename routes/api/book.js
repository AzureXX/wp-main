...
// @route   POST api/book/google
// @desc    Adds new book to database via google book api
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const response = await axios.get('https://www.googleapis.com/books/v1/volumes/' + req.body.id);
      const description = req.body.withDescription
        ? sanitizeHTML(response.data.volumeInfo.description, {
            allowedTags: []
          })
        : null;
