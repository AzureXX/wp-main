...
router.get('/get/all/:page?', roles.isUser, async (req, res, next) => {
  await requests.getAllItems(req, res, next, EducationTopic, 'topic', EducationTopicRating, 20);
});
...
router.get('/get/id/:id', roles.isUser, async (req, res, next) => {
  await requests.getItem(req, res, next, EducationTopic, 'topic', EducationTopicRating);
});
